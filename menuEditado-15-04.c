	...
	if(opcion1<1||opcion1>5){
		...
		switch (opcion1) {
			...
			case 4:
				...
				printf("Introduzca la contrasenia de empleados:");
				do{
					scanf("%d", &contrasenia);
					...
					if(password==contrasenia){
						correcto=0;
						system("cls");
						menuEmpleados();//AAAAAAAAAAAAAAAAAAAAAAAAAAA
						scanf("%d",&opcion3);
				...
				case 1: 
	    			...
	    			fclose(pfichero); 
	   				printf("\nPara volver al menu de empleados presione 'S'");
	   	 			scanf("%c", &opcion);
	   	 			if(opcion == 'S'){
	   	 				menuPrincipal();
						}
				...
				return 0;
				break; //AAAAAAAAAAAAAAAAA
				}
				}
				else{
					correcto=1;
					printf("Contrasenia incorrecta, vuelva a intentarlo:\n");
				}
...
*/
	printf("1-Inventario\n");
	printf("2-Aniadir articulos\n");
	printf("3-Restar articulos\n");
	printf("4-Registro de empleados\n");
	printf("5-Salir");
	printf("\nIngrese su opcion: [ ]\b\b");
...
void 	portalEmpleados(){
	FILE * pfichero;
	int contador=0, contrasenia;
	struct trabajo empleados[100];
	int password=1234;
	char Registro[100];
	int existe, i, correcto=1;*/
/*	pfichero = fopen("empleados.txt", "r");
